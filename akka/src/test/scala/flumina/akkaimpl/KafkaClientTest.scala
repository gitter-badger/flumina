...
abstract class KafkaClientTest extends TestKit(ActorSystem())
  ...
  "KafkaClient" should {
    ...
    //      val name1 = randomTopic(partitions = nrPartitions, replicationFactor = 1)
    //      val name2 = randomTopic(partitions = 1, replicationFactor = 1)
    //      val size = 5000
    //
    //      Source(1 to size)
    //        .map(x => TopicPartition(name1, x % nrPartitions) -> Record.fromByteValue(Seq(x.toByte)))
    //        .runWith(client.producer(100, 1))
    //
    //      client.consume(s"${group}_a", name1, nrPartitions)
    ...
    //        .ensureSubscription()
    //        .request(2500)
    //        .expectNextN(2500) should have size 2500
    //    }
